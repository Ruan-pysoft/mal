...
static Value_own
eval_quote(List_own args, MutEnv_ref env, rerr_t ref err_out)
{
	rerr_t err = RERR_OK;
	Value_own res;
	...
	res = value_copy(list_nth(args, 0, NULL));
	list_free(args);

	return res;
}
static Value_own quasiquote(List_own args, MutEnv_ref env, rerr_t ref err_out);
static List_own
quasiquote_list(List_ref list, usz from, MutEnv_ref env, rerr_t ref err_out)
{
	rerr_t err = RERR_OK;
	Value_ref elt;
	String_own str;
	Value_own val;
	List_own quoted, args;
	List_own res;

	if (from >= list_len(list)) {
		res = list_new(NULL, 0, &err.e.errt);
		RTRY_WITH(err, NULL);
		return res;
	}

	elt = list_nth(list, from, NULL);
	if (value_islist(elt) && list_len(value_getlist(elt, NULL)) > 0) {
		Value_ref head = list_nth(value_getlist(elt, NULL), 0, NULL);

		if (value_issymbol(head) && string_match(
			value_getsymbol(head, NULL), "splice-unquote"
		) == C_EQ) {
			if (list_len(value_getlist(elt, NULL)) != 2) {
				err = rerr_arg_len_mismatch(
					2, list_len(value_getlist(elt, NULL))
				);
				RERR_WITH(err, NULL);
			}
			...
			res = list_new(NULL, 0, &err.e.errt);
			RTRY_WITH(err, NULL);
			str = string_new("concat", &err.e.errt);
			if (!rerr_is_ok(err)) {
				list_free(res);
				RERR_WITH(err, NULL);
			}
			...
			_list_append(
				(struct List_struct ref)res,
				value_copy(list_nth(
					value_getlist(elt, NULL), 1, NULL
				)), &err.e.errt
			);
			RTRY_WITH(err, NULL);

			quoted = quasiquote_list(list, from+1, env, &err);
			if (!rerr_is_ok(err)) {
				list_free(res);
				RERR_WITH(err, NULL);
			}
			val = value_list(quoted, &err.e.errt);
			if (!rerr_is_ok(err)) {
				list_free(res);
				RERR_WITH(err, NULL);
			}
	...
	res = list_new(NULL, 0, &err.e.errt);
	RTRY_WITH(err, NULL);
	str = string_new("cons", &err.e.errt);
	if (!rerr_is_ok(err)) {
		list_free(res);
		RERR_WITH(err, NULL);
	}
	...
	RTRY_WITH(err, NULL);
	args = list_new(&elt, 1, &err.e.errt);
	if (!rerr_is_ok(err)) {
		list_free(res);
		RERR_WITH(err, NULL);
	}
	val = quasiquote(args, env, &err);
	if (!rerr_is_ok(err)) {
		list_free(res);
		RERR_WITH(err, NULL);
	}
...
static Value_own
quasiquote(List_own args, MutEnv_ref env, rerr_t ref err_out)
{
	rerr_t err = RERR_OK;
	Value_own ast;
	...
	ast = value_copy(list_nth(args, 0, NULL));
	list_free(args);

	switch (value_type(ast)) {
		case VT_LST: {
			List_ref ast_list = value_getlist(ast, NULL);
			Value_ref head;
			List_own list;
			Value_own res;

			if (list_len(ast_list) == 0) {
				return ast;
			}

			head = list_nth(ast_list, 0, NULL);
			if (value_issymbol(head) && string_match(
				value_getsymbol(head, NULL), "unquote"
			) == C_EQ) {
				if (list_len(ast_list) != 2) {
					err = rerr_arg_len_mismatch(
						2, list_len(ast_list)
					);
					value_free(ast);
				...
				res = value_copy(list_nth(ast_list, 1, NULL));
				value_free(ast);
				return res;
			}

			list = quasiquote_list(ast_list, 0, env, &err);
			value_free(ast);
			RTRY_WITH(err, NULL);
		...
		case VT_SYM: {
			...
			str = string_new("quote", &err.e.errt);
			if (!rerr_is_ok(err)) {
				list_free(list);
				value_free(ast);
				RERR_WITH(err, NULL);
...
static struct tailcall_info
eval_quasiquote(List_own args, MutEnv_ref env, rerr_t ref err_out)
{
	struct tailcall_info res;
	res.env = NULL;
	res.tailcall = true;

	res.val = quasiquote(args, env, err_out);
	return res;
}
		...
		if (value_islist(val)) {
			...
			if (value_issymbol(head)) {
				...
				if (string_match(fnname, "def!") == C_EQ) {
					...
					RTRY_WITH(err, NULL);
					return res;
				} else if (string_match(fnname, "quote") == C_EQ) {
					value_free(head);

					res = eval_quote(tail, env, &err);
					env_free(env);
					RTRY_WITH(err, NULL);
					return res;
				} else if (string_match(fnname, "quasiquote") == C_EQ) {
					value_free(head);

					tci = eval_quasiquote(tail, env, &err);
					TCO();
				}
			}
